...
import { readJsonlHead, readJsonlTail, readAiTitle } from '../util/jsonlReader';
...
export class SessionScanner {
  ...
  async scanSessions(
    ...
    for (const file of jsonlFiles) {
      ...
      // Get summary: index → head/tail events → grep ai-title from full file
      const indexEntry = summaryMap.get(sessionId);
      let rawSummary = indexEntry?.summary ?? this.deriveSummary(headEvents, tailEvents);
      if (!rawSummary) {
        rawSummary = await readAiTitle(path.join(sessionsDir, file)) ?? '';
      }
  ...
  /**
   * Derive a summary from head + tail events. Priority:
   *   1. ai-title event (Claude's auto-generated title) — check both head and tail
   *   2. First user message text — check head first, then tail
   */
  private deriveSummary(
    headEvents: JsonlEvent[],
    tailEvents: JsonlEvent[],
  ): string {
    // Check for ai-title in head (early sessions) and tail (recent/long sessions)
