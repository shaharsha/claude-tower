...
function makeMetaUserMessage(rawContent: string): any {
  return {
    type: 'user',
...
describe('scanSessions', () => {
  ...
  it('skips isMeta user turns when deriving summary', async () => {
    writeJsonl('meta1111-1111-2222-3333-444444444444.jsonl', [
      makeSessionStart('meta1111-1111-2222-3333-444444444444'),
      makeMetaUserMessage(
        '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands…</local-command-caveat>',
      ),
      makeUserMessage('actual user request'),
      makeAssistantEnd(),
    ]);
    ...
    assert.equal(sessions.length, 1);
    assert.equal(sessions[0].summary, 'actual user request');
  });

  it('skips tag-wrapped user content even when isMeta flag is missing', async () => {
    writeJsonl('meta2222-1111-2222-3333-444444444444.jsonl', [
      makeSessionStart('meta2222-1111-2222-3333-444444444444'),
      // no isMeta flag — older logs may omit it; the tag-wrapping heuristic still catches it
      {
        type: 'user',
        message: { role: 'user', content: '<ide_opened_file>The user opened the file /tmp/foo.ts</ide_opened_file>' },
        timestamp: new Date().toISOString(),
      },
      makeUserMessage('real task here'),
      makeAssistantEnd(),
    ]);
    ...
    assert.equal(sessions.length, 1);
    assert.equal(sessions[0].summary, 'real task here');
  });

  it('keeps text that opens with < but is not a closed tag wrapper', async () => {
    writeJsonl('lit33333-1111-2222-3333-444444444444.jsonl', [
      makeSessionStart('lit33333-1111-2222-3333-444444444444'),
      makeUserMessage('<TodoWrite> should I use this tool here?'),
      makeAssistantEnd(),
    ]);
    ...
    assert.equal(sessions.length, 1);
    assert.equal(sessions[0].summary, '<TodoWrite> should I use this tool here?');
  });

  it('returns empty summary when every user turn is meta', async () => {
    writeJsonl('all4meta-1111-2222-3333-444444444444.jsonl', [
      makeSessionStart('all4meta-1111-2222-3333-444444444444'),
      makeMetaUserMessage('<local-command-caveat>x</local-command-caveat>'),
      makeMetaUserMessage('<command-name>/compact</command-name>'),
      makeAssistantEnd(),
    ]);
